#!/usr/bin/env python3
"""
Django Revolution Publisher CLI
...
def main():
    console.print(Panel("[bold blue]Django Revolution Publisher[/bold blue]\nInteractive package publishing to PyPI", title="🚀 PyPI Publisher", border_style="blue"))
    ...
    bump_version = questionary.confirm(
        "Do you want to bump the version before publishing?",
        default=True
    ).ask()
    ...
    if bump_version:
        ...
        if bump_type:
            try:
                new_version = version_config.bump_version(bump_type)
                console.print(f"[green]✅ Version bumped to: {new_version}[/green]")
                
                # Validate version consistency
                if not version_config.validate_version_consistency():
                    console.print("[red]❌ Version inconsistencies found! Please fix before publishing.[/red]")
                    return 1
    ...
    confirm = questionary.confirm(
        f"Publish to {'PyPI' if repo == 'pypi' else 'TestPyPI'}?",
        default=True
    ).ask()
    if not confirm:
        console.print("❌ Publishing cancelled.")
    ...
    build_result = subprocess.run([sys.executable, "-m", "build"], capture_output=True, text=True)
